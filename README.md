# «Топливный космоконтур 2035»: стартовый технический репозиторий кейса
...
Этот GitHub-репозиторий содержит **контрольные правила, форматы обмена, синтетические тест-векторы, научную базу и порядок проверки**. Здесь **нет готовой конкурсной стратегии**. Репозиторий не выбирает за команду source mix, объёмы закупки, момент инвестиций, метод оптимизации, программный стек или интерфейс.
...
30. [Как жюри проверяет систему](#30-как-жюри-проверяет-систему)
31. [Карта критериев 100 + 5](#31-карта-критериев-100--5)
32. [Геополитический бонусный модуль](#32-геополитический-бонусный-модуль)
33. [Типовые ошибки](#33-типовые-ошибки)
34. [Структура репозитория команды](#34-структура-репозитория-команды)
35. [Научная база](#35-научная-база)
36. [Происхождение данных](#36-происхождение-данных)
37. [Финальная проверка перед защитой](#37-финальная-проверка-перед-защитой)
38. [Глоссарий](#38-глоссарий)
...
Обязательная часть не требует детальной trajectory optimization, выбора конкретной ракеты, CFD, геометрии баков или телеметрии. Команда может добавить higher-fidelity module как исследование, но он не заменяет контрольный material/financial balance.
...
| Optional geopolitical module | +5 | отдельный проверяемый research scenario |

Основная шкала — 100, максимальный итог с бонусом — 105.
...
**Starter repository `test_oil` намеренно устроен иначе:** здесь нет `src/` с готовым participant engine, `app.py`, готового optimizer или заполненного competition plan. Всё это должны сделать участники.
...
## 34. Происхождение данных

Основная постановка ссылается на `Анкета_постановщика_КосмоХакатон_ЕТ.xlsx`, лист `Данные кейса 2`. Этот XLSX не был предоставлен в комплекте, использованном для starter repository.

Поэтому текущий `data/` транскрибирован из предоставленных:
...
Все трактовки и ограничения перечислены в [`docs/ERRATA_AND_PROVENANCE.md`](docs/ERRATA_AND_PROVENANCE.md). Если исходный XLSX появится, проводится reconciliation до release, а не молчаливое предположение, что файлы совпадают.
